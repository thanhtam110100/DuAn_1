...
public class LoaiSPFrame extends javax.swing.JFrame {
    ...
    public LoaiSPFrame() {
        ...
        loadData(iLoaiSPService.getAll(0));
        loadDataDaXoa();
        _index = -1;
    }

    private void loadData(List<QLLoaiSP> lst) {
        model = (DefaultTableModel) tblQLLoaiSP.getModel();
        model.setRowCount(0);
        int count = 1;
        for (QLLoaiSP loai : lst) {
            model.addRow(new Object[]{count++, loai.getMa(), loai.getTen()});
        }
    }
    ...
    private void loadDataDaXoa() {
        ...
        model.setRowCount(0);
        int count = 1;
        List<QLLoaiSP> lst = iLoaiSPService.getAll(1);
        for (QLLoaiSP loai : lst) {
            model.addRow(new Object[]{count++, loai.getMa(), loai.getTen()});
        }
    }
    ...
    private void initComponents() {
        ...
        tblQLLoaiSP = new javax.swing.JTable();
        ...
        tblQLLoaiSP.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {

            },
            new String [] {
                "STT", "Mã", "Tên loại sản phẩm"
            }
        ));
        tblQLLoaiSP.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                tblQLLoaiSPMouseClicked(evt);
            }
        });
        jScrollPane1.setViewportView(tblQLLoaiSP);
        ...
        jLabel1.setText("Mã:");
        jPanel1.add(jLabel1);
        jLabel1.setBounds(20, 320, 40, 20);

        jLabel2.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        jLabel2.setText("Tên loại sản phẩm:");
        jPanel1.add(jLabel2);
        jLabel2.setBounds(20, 380, 120, 20);
        jPanel1.add(txtMa);
        txtMa.setBounds(150, 320, 150, 27);
        jPanel1.add(txtTen);
        txtTen.setBounds(150, 380, 150, 27);
        ...
        layout.setHorizontalGroup(
            ...
            .addComponent(jTabbedPane1, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.PREFERRED_SIZE, 521, javax.swing.GroupLayout.PREFERRED_SIZE)
        );
    ...
    private void btnThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemActionPerformed
        ...
        List<QLLoaiSP> lst = iLoaiSPService.find(ma);
        if (lst.size() > 0) {
            JOptionPane.showMessageDialog(this, "Mã đã tồn tại");
            return;
        }
        for (int i = 0; i < iLoaiSPService.getAll(0).size(); i++) {
            if (txtTen.getText().equals(iLoaiSPService.getAll(0).get(i).getTen())) {
                JOptionPane.showMessageDialog(this, "Tên sản phẩm không được trùng");
                return;
            }
        }
        QLLoaiSP loai = new QLLoaiSP(ma, ten);
        if (iLoaiSPService.insert(loai)) {
    ...
    private void btnSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSuaActionPerformed
        ...
        _index = tblQLLoaiSP.getSelectedRow();
        String id = iLoaiSPService.getAll(0).get(_index).getId();
        String ma = txtMa.getText().trim();
        String ten = txtTen.getText().trim();
        ...
        String txtMa = iLoaiSPService.getAll(0).get(_index).getMa();
        List<QLLoaiSP> lst = iLoaiSPService.getAll(0);
        ...
        for (int i = 0; i < iLoaiSPService.getAll(0).size(); i++) {
            if (txtTen.getText().equals(iLoaiSPService.getAll(0).get(i).getTen())
                    && !iLoaiSPService.getAll(0).get(_index).getTen().equals(iLoaiSPService.getAll(0).get(i).getTen())) {
                JOptionPane.showMessageDialog(this, "Tên sản phẩm không được trùng");
                return;
            }
        }
        QLLoaiSP loai = new QLLoaiSP(ma, ten);
        if (iLoaiSPService.update(loai, id)) {
    ...
    private void tblQLLoaiSPMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblQLLoaiSPMouseClicked
        _index = tblQLLoaiSP.getSelectedRow();
        txtMa.setText(tblQLLoaiSP.getValueAt(_index, 1).toString());
        txtTen.setText(tblQLLoaiSP.getValueAt(_index, 2).toString());
    }//GEN-LAST:event_tblQLLoaiSPMouseClicked

    private void btnXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXoaActionPerformed
        _index = tblQLLoaiSP.getSelectedRow();
        if (_index == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn loại sản phẩm muốn xóa");
            return;
        }
    ...
    private void btnTimKiemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTimKiemActionPerformed
        ...
        List<QLLoaiSP> lst = iLoaiSPService.find(timKiem);
        loadData(lst);
        if (lst.size() == 0) {
            JOptionPane.showMessageDialog(this, "Không tìm thấy loại sản phẩm " + timKiem);
        }
        clearForm();
    ...
    private void btnKhoiPhucActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnKhoiPhucActionPerformed
        ...
        if (_index == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn loại sản phẩm muốn khôi phục");
            return;
        }
        ...
        if (xacNhan != JOptionPane.YES_OPTION) {
            ...
            String id = iLoaiSPService.getAll(1).get(_index).getId();
            if (iLoaiSPService.delete(0, id)) {
    ...
    private javax.swing.JTable tblQLLoaiSP;
