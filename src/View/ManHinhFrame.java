...
public class ManHinhFrame extends javax.swing.JFrame {
    ...
    public ManHinhFrame() {
        ...
        loadData(iManHinhService.getAll(0));
        loadDataDaXoa();
        _index = -1;
    }

    private void loadData(List<QLManHinh> lst) {
    ...
    private void loadDataDaXoa() {
        ...
        model.setRowCount(0);
        int count = 1;
        List<QLManHinh> lst = iManHinhService.getAll(1);
        for (QLManHinh h : lst) {
            model.addRow(new Object[]{count++, h.getMa(), h.getTen()});
        }
    }
    ...
    private void initComponents() {
        ...
        jPanel1.add(jLabel2);
        jLabel2.setBounds(30, 380, 100, 20);
        jPanel1.add(txtMa);
        txtMa.setBounds(130, 320, 150, 27);
        jPanel1.add(txtTen);
        txtTen.setBounds(130, 380, 150, 27);
        ...
        jTabbedPane1.addTab("Màn hình", jPanel1);
    ...
    private void btnThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemActionPerformed
        ...
        List<QLManHinh> lst = iManHinhService.find(ma);
        if (lst.size() > 0) {
            JOptionPane.showMessageDialog(this, "Mã đã tồn tại!");
            return;
        }
        for (int i = 0; i < iManHinhService.getAll(0).size(); i++) {
            if (txtTen.getText().equals(iManHinhService.getAll(0).get(i).getTen())) {
                JOptionPane.showMessageDialog(this, "Tên màn hình không được trùng");
                return;
            }
        }
        QLManHinh c = new QLManHinh(ma, ten);
        if (iManHinhService.insert(c)) {
    ...
    private void btnSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSuaActionPerformed
        if (_index == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn màn hình muốn sửa");
            return;
        }
        _index = tblManHinh.getSelectedRow();
        String id = iManHinhService.getAll(0).get(_index).getId();
        String ma = txtMa.getText().trim();
        String ten = txtTen.getText().trim();
        if (ma.length() == 0 || ten.length() == 0) {
            JOptionPane.showMessageDialog(this, "Vui lòng điền đủ thông tin các trường");
        ...
        String txtMa = iManHinhService.getAll(0).get(_index).getMa();
        List<QLManHinh> lst = iManHinhService.getAll(0);
        ...
        for (int i = 0; i < iManHinhService.getAll(0).size(); i++) {
            if (txtTen.getText().equals(iManHinhService.getAll(0).get(i).getTen())
                    && !iManHinhService.getAll(0).get(_index).getTen().equals(iManHinhService.getAll(0).get(i).getTen())) {
                JOptionPane.showMessageDialog(this, "Tên màn hình không được trùng");
                return;
            }
        }
        QLManHinh c = new QLManHinh(ma, ten);
        if (iManHinhService.update(c, id)) {
    ...
    private void tblManHinhMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblManHinhMouseClicked
        _index = tblManHinh.getSelectedRow();
        txtMa.setText(tblManHinh.getValueAt(_index, 1).toString());
        txtTen.setText(tblManHinh.getValueAt(_index, 2).toString());
    }//GEN-LAST:event_tblManHinhMouseClicked

    private void btnXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXoaActionPerformed
        _index = tblManHinh.getSelectedRow();
        if (_index == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn màn hình muốn xóa");
            return;
        }
        ...
        if (xacNhan != JOptionPane.YES_OPTION) {
            ...
            String id = iManHinhService.getAll(0).get(_index).getId();
            if (iManHinhService.delete(1, id)) {
    ...
    private void btnTimKiemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTimKiemActionPerformed
        ...
        List<QLManHinh> lst = iManHinhService.find(timKiem);
        loadData(lst);
        if (lst.size() == 0) {
            JOptionPane.showMessageDialog(this, "Không tìm thấy màn hình " + timKiem);
        }
        clearForm();
    ...
    private void btnKhoiPhucActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnKhoiPhucActionPerformed
        ...
        if (_index == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn màn hình muốn khôi phục");
            return;
        }
        ...
        if (xacNhan != JOptionPane.YES_OPTION) {
            ...
            String id = iManHinhService.getAll(1).get(_index).getId();
            if (iManHinhService.delete(0, id)) {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ManHinhFrame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
