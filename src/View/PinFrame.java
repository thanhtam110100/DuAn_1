...
public class PinFrame extends javax.swing.JFrame {
    ...
    public PinFrame() {
        ...
        loadData(iPinService.getAll(0));
        loadDataDaXoa();
        _index = -1;
    }

    private void loadData(List<QLPin> lst) {
        model = (DefaultTableModel) tblQLPin.getModel();
        model.setRowCount(0);
        int count = 1;
        for (QLPin h : lst) {
    ...
    private void loadDataDaXoa() {
        ...
        model.setRowCount(0);
        int count = 1;
        List<QLPin> lst = iPinService.getAll(1);
        for (QLPin h : lst) {
            model.addRow(new Object[]{count++, h.getMa(), h.getTen()});
        }
    }
    ...
    private void initComponents() {
        ...
        tblQLPin = new javax.swing.JTable();
        ...
        tblQLPin.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        tblQLPin.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                tblQLPinMouseClicked(evt);
            }
        });
        jScrollPane1.setViewportView(tblQLPin);
        ...
        jTabbedPane1.addTab("Pin", jPanel1);
    ...
    private void btnThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemActionPerformed
        ...
        List<QLPin> lst = iPinService.find(ma);
        if (lst.size() > 0) {
            JOptionPane.showMessageDialog(this, "Mã đã tồn tại");
            return;
        }
        for (int i = 0; i < iPinService.getAll(0).size(); i++) {
            if (txtTen.getText().equals(iPinService.getAll(0).get(i).getTen())) {
                JOptionPane.showMessageDialog(this, "Tên Pin không được trùng");
                return;
            }
        }
        QLPin c = new QLPin(ma, ten);
        if (iPinService.insert(c)) {
    ...
    private void btnSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSuaActionPerformed
        if (_index == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn pin muốn sửa");
            return;
        }
        _index = tblQLPin.getSelectedRow();
        String id = iPinService.getAll(0).get(_index).getId();
        String ma = txtMa.getText().trim();
        String ten = txtTen.getText().trim();
        if (ma.length() == 0 || ten.length() == 0) {
            JOptionPane.showMessageDialog(this, "Vui lòng điền đủ thông tin các trường");
        ...
        String txtMa = iPinService.getAll(0).get(_index).getMa();
        List<QLPin> lst = iPinService.getAll(0);
        ...
        for (int i = 0; i < iPinService.getAll(0).size(); i++) {
            if (txtTen.getText().equals(iPinService.getAll(0).get(i).getTen())
                    && !iPinService.getAll(0).get(_index).getTen().equals(iPinService.getAll(0).get(i).getTen())) {
                JOptionPane.showMessageDialog(this, "Tên Pin không được trùng");
                return;
            }
        }
        QLPin c = new QLPin(ma, ten);
        if (iPinService.update(c, id)) {
    ...
    private void tblQLPinMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblQLPinMouseClicked
        _index = tblQLPin.getSelectedRow();
        txtMa.setText(tblQLPin.getValueAt(_index, 1).toString());
        txtTen.setText(tblQLPin.getValueAt(_index, 2).toString());
    }//GEN-LAST:event_tblQLPinMouseClicked

    private void btnXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXoaActionPerformed
        _index = tblQLPin.getSelectedRow();
        if (_index == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn Pin muốn xóa");
            return;
        }
    ...
    private void btnTimKiemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTimKiemActionPerformed
        ...
        List<QLPin> lst = iPinService.find(timKiem);
        loadData(lst);
        if (lst.size() == 0) {
            JOptionPane.showMessageDialog(this, "Không tìm thấy pin " + timKiem);
        }
        clearForm();
    ...
    private void btnKhoiPhucActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnKhoiPhucActionPerformed
        ...
        if (_index == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn Pin muốn khôi phục");
            return;
        }
        ...
        if (xacNhan != JOptionPane.YES_OPTION) {
            ...
            String id = iPinService.getAll(1).get(_index).getId();
            if (iPinService.delete(0, id)) {
    ...
    private javax.swing.JTable tblQLPin;
